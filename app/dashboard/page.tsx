    ...
    return (
      ...
      <div className="w-full max-w-fit bg-[#654bf1] -mt-20 p-5 rounded-lg flex flex-col md:flex-row gap-5 shadow-lg">
        ...
        <div className="md:w-1/2 w-full flex flex-col items-center gap-6 rounded-2xl mt-16 md:mt-0">
          ...
          {/* Local video preview */}
          <div className="w-full flex-1 h-full flex items-center justify-center rounded-2xl">
            ...
            {/* {!inVideoChat && (
              <>
                <div className="flex gap-3 mb-4 justify-center">
                  <button
                    onClick={() => {
                      // always show upgrade modal when selecting a specific gender
                      setShowUpgradeModal(true);
                      if (isPremium) setPreference("male");
                    }}
                    className={`px-4 py-2 rounded-lg transition-all ${preference === "male" ? "bg-blue-600" : "bg-gray-700"} ${!isPremium ? "opacity-50 cursor-not-allowed" : "hover:bg-blue-500"}`}
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-all ${preference === "female" ? "bg-pink-600" : "bg-gray-700"} ${!isPremium ? "opacity-50 cursor-not-allowed" : "hover:bg-pink-500"}`}
                    title={!isPremium ? "Premium required" : "Filter by females"}
                  >
                    Girls {!isPremium && "🔒"}
                  </button>

                  <button onClick={() => setPreference("both")} className={`px-4 py-2 rounded-lg transition-all ${preference === "both" ? "bg-green-600" : "bg-gray-700"} hover:bg-green-500`} title="Match with anyone">
                    Both
                  </button>
                </div>

                <div className="flex justify-center mb-3">
                  <button onClick={handleEnterChat} disabled={searching} className={`${searching ? "bg-blue-400" : "bg-blue-600 hover:bg-blue-700"} text-white px-8 py-3 rounded-xl text-lg font-semibold transition-all duration-300`}>
                    {searching ? (
        ...
        {/* Right column: remote video + chat */}
         {!inVideoChat && (
          <div className="md:w-1/2 w-full bg-gradient-to-b from-[#6b4fd4] to-[#5940df] h-[400px] flex flex-col justify-between items-center rounded-3xl p-8">
        ...
        <div className=" w-full h-full flex md:flex-row flex-col">
          ...
          <div className="min-w-[320px] h-[400px] relative bg-[#5940df] flex items-center justify-center">
            {/* Remote video area */}
